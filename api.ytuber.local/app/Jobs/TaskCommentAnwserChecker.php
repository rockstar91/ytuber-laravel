...
class TaskCommentAnwserChecker implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $commentToCompleteId = $currentTask->extend['id'];
        $commentForUser = \App\Models\TasksComments::where('task_id',$currentTask->id)->where('account_id',$activeAccount->id)->first();

        //ссылка на канал
        $check = parse_url($activeAccount->url);
        ...
        $apiUrl = 'https://youtube.googleapis.com/youtube/v3/commentThreads?part=replies&id=' . $commentToCompleteId . '&key=' . $apikey;
        $comments = Http::get($apiUrl);

        foreach($comments['items'][0]['replies']['comments'] as $c){
            if($c['snippet']['authorChannelId']['value'] == $channel_id){
                if($c['snippet']['textDisplay'] == $commentForUser->comment_text){
                    $status = true;
                }
            }
        }
        ...
        if($status && $complete->status != 4) {
            ...
            $data_for_broadcast = [
                ...
                'text'=>'Выполнил ответ к комментарию',
